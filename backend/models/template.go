...
type ThemeLibrary struct {
	...
	Categories    datatypes.JSON `json:"categories" gorm:"type:jsonb;not null;default:'[]';comment:视觉样式分类（JSON字符串数组）"`
...
type TemplateLibrary struct {
	ID             string         `json:"id" gorm:"type:uuid;primaryKey;default:gen_random_uuid();comment:模板唯一标识（UUID v4）"`
	...
	Categories     datatypes.JSON `json:"categories" gorm:"type:jsonb;not null;default:'[]';comment:行业与岗位分类"`
	Highlights     datatypes.JSON `json:"highlights" gorm:"type:jsonb;not null;default:'[]';comment:模板亮点"`
	Content        datatypes.JSON `json:"content" gorm:"type:jsonb;not null;comment:ResumeData 内容快照"`
	DefaultThemeID string         `json:"default_theme_id" gorm:"type:uuid;not null;index;comment:默认主题ID"`
