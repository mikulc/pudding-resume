...
export function lockModalScroll() {
  ...
  // remains identical on pages that scroll at the document or container level.
  const viewportScrollbarWidth = Math.max(0, window.innerWidth - root.clientWidth);
  const scrollbarWidth = Math.max(viewportScrollbarWidth, measureNativeScrollbarWidth());
