...
import { ArrowRight, FileText, LayoutTemplate, Loader2, X } from 'lucide-react';
...
export function CreateResumeModal({ open, onClose }: CreateResumeModalProps) {
  ...
  const handleCreate = useCallback(async () => {
    ...
    const layoutId = DEFAULT_LAYOUT_ID;
    ...
    try {
      const resumeData = await removeUnavailableDefaultAvatar(createEmptyResumeData());
  ...
  }, [isCreating, navigate, showToast, t]);
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative z-10 flex max-h-[90vh] w-[900px] max-w-[calc(100vw-32px)] flex-col overflow-hidden rounded-[22px] bg-white shadow-[0_24px_70px_rgba(15,23,42,0.22)] animate-in fade-in zoom-in-95 duration-200 dark:bg-[color:var(--bg-card)] dark:shadow-[0_24px_70px_rgba(23,25,29,0.45)]">
        <button
          type="button"
          onClick={handleClose}
        ...
        <div className="create-resume-scrollbar flex-1 space-y-5 overflow-y-auto px-6 pb-6 sm:px-8">
          <section>
            <div
            ...
            >
              <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-gray-50 text-gray-500 transition-colors group-hover:bg-white group-hover:text-[var(--theme-accent)] dark:bg-white/[0.06] dark:text-gray-400 dark:group-hover:bg-white/[0.1]">
              ...
              <button
                ...
                className="inline-flex h-10 shrink-0 items-center justify-center gap-2 rounded-xl bg-[var(--theme-accent)] px-4 text-sm font-semibold text-[var(--theme-accent-foreground)] transition-all hover:opacity-90 active:scale-[0.98] sm:min-w-[112px]"
              >
                {t('create.createNow')}
          ...
          <section>
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <h3 className="resume-card-title text-base font-semibold text-gray-900 dark:text-gray-100">
                  {t('create.fromTemplate.title')}
                </h3>
                <p className="mt-0.5 max-w-[520px] text-sm leading-5 text-gray-500 dark:text-gray-400">
                  {t('create.fromTemplate.description')}
                </p>
              </div>
              ...
              <button
                ...
                className="inline-flex h-10 shrink-0 items-center justify-center gap-2 rounded-xl bg-[var(--theme-accent)] px-4 text-sm font-semibold text-[var(--theme-accent-foreground)] transition-all hover:opacity-90 active:scale-[0.98] sm:min-w-[112px]"
              >
                {t('create.fromTemplate.browse', { defaultValue: t('templatesPage.title') })}
                <ArrowRight className="h-4 w-4" />
              </button>
            </div>
          </section>
